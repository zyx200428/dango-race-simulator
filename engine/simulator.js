...
// 爱弥斯最终直播观察版：经过第18格只记录 hasPassedMidpoint；仅在爱弥斯自己的主动行动结束后检查是否传送。
...
function moveForward(s,ids,steps,actorId,label){ if(!ids.length||s.winner) return; const from=getIdTile(s,ids[0]); if(s.lastAction){ s.lastAction.groupIds=[...ids]; s.lastAction.path=[from]; } const oldStack=stackAt(s,from); setStack(s,from,oldStack.filter(id=>!ids.includes(id))); let to=from; for(let i=0;i<steps;i++){ to=nextTile(to); if(s.lastAction) s.lastAction.path.push(to); } if(steps>0 && passesTileForward(from,steps,1)){
  ...
  if(type==="裂隙"){ const st=stackAt(s,to); const hasKing=st.includes("king"); const dangos=st.filter(isDango); const shuffled=shuffle(dangos,s.rng); setStack(s,to,hasKing?[...shuffled,"king"]:shuffled); s.log.push(`${label} 落在第${to}格裂隙，该格堆叠随机重排。`); return; }
  if(type==="推进"||type==="阻遏"){ let extra=type==="推进"?1:-1; if(actorId==="roccia") extra += type==="推进"?3:-1; s.log.push(`${label} 触发第${to}格${type}，${extra>0?"前进":"后退"}${Math.abs(extra)}格。`); const group=stackAt(s,to).filter(id=>ids.includes(id)); setStack(s,to,stackAt(s,to).filter(id=>!group.includes(id))); let finalTile=to; for(let i=0;i<Math.abs(extra);i++){ finalTile=extra>0?nextTile(finalTile):prevTile(finalTile); if(s.lastAction) s.lastAction.path.push(finalTile); } mergeMovingGroupOnTile(s,group,finalTile,"normal"); }
}
...
function findNearestDangoAhead(s, fromTile, selfId){
  // 爱弥斯只检查当前位置下一格到第1格终点之间的前方目标；同格、所在堆叠、布大王不算目标，也不越过终点继续搜索。
...
function getAemisStatus(s){ return s.group==="B" && s.status ? s.status.aemis : null; }
function attemptAemisTeleport(s, reason=""){
  const st=getAemisStatus(s);
  if(!st || s.winner || st.ghostUsed || !st.hasPassedMidpoint) return false;
  ...
  s.log.push(`${prefix}爱弥斯前方暂时没有目标，电子幽灵保持等待，不消耗技能。`);
  return false;
}
function processAemisMidpointAfterMove(s){
  const st=getAemisStatus(s);
  if(!st || s.winner || st.ghostUsed) return;
  ...
  if(!st.hasPassedMidpoint){
    s.log.push(`爱弥斯经过第${AEMIS_MIDPOINT}格中点，电子幽灵进入待触发状态；将在爱弥斯主动行动结束后检查目标。`);
  }
...
function checkAemisWaitingAfterAction(s){
  const st=getAemisStatus(s);
  if(!st || s.winner || st.ghostUsed || !st.hasPassedMidpoint) return false;
  if(s.lastAction?.actor !== "aemis") return false;
  return attemptAemisTeleport(s, "爱弥斯主动行动结束");
}
...
function rowsFromStats(stats, done){
  return DANGOS.map(d=>{
    const s=stats[d.id];
    ...
    return {...d,wins:s.wins,top4:s.top4,rankSum:s.rankSum,winRate,advanceRate,elimRate,avgRank,rate:winRate};
  }).sort((a,b)=>{
  ...
  function chunk(){
    ...
    for(let i=done;i<end;i++){
      ...
      fr.forEach((id,idx)=>{
        ...
        if(idx<4) stats[id].top4++;
        stats[id].rankSum += idx+1;
      });
    }
    done=end;
    const rows=rowsFromStats(stats,done);
...
function makeSecondHalfState(seed, ranking = getRankingSource(DEFAULT_RANKING_KEY).ranking, sourceLabel = getRankingSource(DEFAULT_RANKING_KEY).label, camellyaEarlyTrigger=false){
  ...
  const status = Object.fromEntries(DANGOS.map(d=>[d.id,{tile:1,lastRoll:null,metKing:false,comeback:false,comebackUsed:false,canFinish:false,rollCount:0,ghostUsed:false,hasPassedMidpoint:false}]));
